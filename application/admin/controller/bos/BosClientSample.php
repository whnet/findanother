...
//Bucket是否存在，若不存在创建Bucket
        ...
        //$this->client->putObject($bucketName, $name, $data,CHAR_MAX,'12345');
        try{
            $this->client->putObjectFromFile($bucketName, $name, $data);//上传
        }
        catch(Exception $e)
        {
        ...
        $url = $this->client->generatePreSignedUrl($bucketName,
            $name,
            array(BosOptions::SIGN_OPTIONS => $signOptions)
        );
        return $url.'&responseContentDisposition=attachment';
        //$this->client->putObjectFromString($bucketName, $name, $data);



    }
    public function geturl($bucketName,$name)
    {
